...
def process_csv(input_file, output_file, max_questions=10, countries=None):
    ...
    # Group by persona and country, limit questions per group
    if max_questions:
        groups = []
...
if __name__ == "__main__":
    # Process persona questions for all countries (sample)
    process_csv(
        "workwithisland_questions_personas.csv", 
        "workwithisland_questions_personas_with_responses.csv",
        max_questions=5,  # Limit to 5 questions per persona/country to reduce API calls
        countries=["France"]  # Process only France initially to save time
    ) 